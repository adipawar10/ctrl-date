...
from app.services.recurrence import expand_recurrence_in_range, add_recurrence_exclusion
...
async def delete_event(
    ...
    """
    # Handle virtual recurring instance IDs (e.g. "<parent_id>_2026-04-20")
    # generated by recurrence expansion.
    parent_id: Optional[str] = None
    instance_date: Optional[date] = None
    lookup_event_id = event_id
    if "_" in event_id:
        maybe_parent, maybe_date = event_id.rsplit("_", 1)
        try:
            UUID(maybe_parent)
            instance_date = date.fromisoformat(maybe_date)
            parent_id = maybe_parent
            lookup_event_id = maybe_parent
        except ValueError:
            parent_id = None
            instance_date = None

    # Fetch existing event (parent for virtual IDs)
    try:
        response = supabase.table("events").select("*").eq(
            "id", lookup_event_id
        ).is_("deleted_at", "null").single().execute()
    except APIError as e:
        if getattr(e, "code", None) == "PGRST116" or "PGRST116" in str(e):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Event not found"
            )
    ...
    # For a virtual instance and single-delete, add an exclusion date on the
    # recurrence rule so that this instance is hidden while preserving the series.
    if parent_id and instance_date and delete_scope == RecurrenceUpdateScope.SINGLE:
        await add_recurrence_exclusion(
            parent_event_id=parent_id,
            exclude_date=instance_date,
            user_id=str(user.id),
        )
        return {"message": "Recurring instance deleted"}
    ...
    if delete_scope == RecurrenceUpdateScope.ALL and existing.get("recurrence_parent_id"):
        ...
        parent_id = existing.get("recurrence_parent_id") or lookup_event_id
    ...
    elif delete_scope == RecurrenceUpdateScope.FUTURE and (
        existing.get("recurrence_instance_date") or instance_date
    ):
        # Delete this and future instances
        parent_ref = existing.get("recurrence_parent_id") or lookup_event_id
        from_date = existing.get("recurrence_instance_date") or (
            instance_date.isoformat() if instance_date else None
        )
        supabase.table("events").update({
            "deleted_at": deleted_at
        }).eq(
            "recurrence_parent_id", parent_ref
        ).gte(
            "recurrence_instance_date", from_date
        ).execute()
    else:
        # Delete single event
        supabase.table("events").update({
            "deleted_at": deleted_at
        }).eq("id", lookup_event_id).execute()
